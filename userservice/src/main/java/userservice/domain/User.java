...
public class User {
  ...
  /**
   * User constructor to instantiate a not yet persisted User with personal coordinates.
   *
  ...
  private User(UUID userId, String userName, String phoneNumber, String emailAddress) {
    this.userId = userId;
    this.userName = userName;
    this.phoneNumber = phoneNumber;
    this.emailAddress = emailAddress;
  ...
  /**
   * Builder pattern to return a User with the provided Id.
   *
   * @param user   to build from
   * @param userId of the user
   * @return new User with id
   */
  public static User of(User user, UUID userId) {
    return new User(
        userId,
        user.userName,
        user.phoneNumber,
